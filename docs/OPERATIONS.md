...
Client certificates (mutual TLS): set `SBC_FS_TLS_VERIFY_POLICY=subjects_in` (or `in` to accept any certificate from the trusted CA), append the CA that issues customer certificates to `cafile.pem`, put each customer's certificate subject (CN or SAN) in its "TLS client certificate subject" field, and restart the ingress profile (System page or `POST /api/v1/system/profiles/external-ingress/restart`; in-flight calls on that profile drop). Connections with an unknown certificate fail at the TLS handshake and never reach the SIP layer.
