...
var (
	...
	carriers = []carrierDef{
		...
		{"carrier-503", "172.28.0.62", 5060, []rateRow{
			{"44", "UK Fixed", "0.005500", "0", 60, 60}, {"447", "UK Mobile", "0.014000", "0.002000", 60, 60}}, nil, 0, "", ""},
	...
	customers = []customerDef{
		...
		{"zeta-tls", "172.28.0.107/32", "100.000000", "0", 0, 0, "", "", true, "", "172.28.0.10"},
...
func Run(ctx context.Context, st *store.Store, log *slog.Logger) error {
	...
	for _, c := range customers {
		cust, err := st.UpsertCustomer(ctx, &model.Customer{
			...
			MediaMode: c.media, SRTPMode: c.srtp, RequireTLS: c.requireTLS, STIRMode: c.stir, TLSSubject: c.tlsSubject,
		})
		if err != nil {
			return err
		}
