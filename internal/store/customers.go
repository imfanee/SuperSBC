...
const customerCols = `id, name, status::text AS status, rate_group_id, route_group_id, max_concurrent_calls, max_cps,
	allowed_codecs, tech_prefix, default_country_code, notes, created_at, updated_at`
...
func (s *Store) CustomerByIP(ctx context.Context, ip string, port int, transport string) (*IPMatch, error) {
	...
	rows, err := s.pool.Query(ctx, `
		SELECT c.id, c.name, c.status::text AS status, c.rate_group_id, c.route_group_id, c.max_concurrent_calls, c.max_cps,
		       c.allowed_codecs, c.tech_prefix, c.default_country_code, c.notes, c.created_at, c.updated_at,
	...
	if err := rows.Scan(&c.ID, &c.Name, &c.Status, &c.RateGroupID, &c.RouteGroupID, &c.MaxConcurrentCalls, &c.MaxCPS,
		&c.AllowedCodecs, &c.TechPrefix, &c.DefaultCountryCode, &c.Notes, &c.CreatedAt, &c.UpdatedAt,
...
func (s *Store) UpsertCustomer(ctx context.Context, c *model.Customer) (*model.Customer, error) {
	return one[model.Customer](ctx, s.pool, `
		INSERT INTO customers (name, status, rate_group_id, route_group_id, max_concurrent_calls, max_cps, allowed_codecs, tech_prefix, default_country_code, notes)
		VALUES ($1, $2::customer_status, $3, $4, $5, $6, $7, $8, $9, $10)
		ON CONFLICT (name) DO UPDATE SET status = EXCLUDED.status, rate_group_id = EXCLUDED.rate_group_id, route_group_id = EXCLUDED.route_group_id,
		  ...
		  tech_prefix = EXCLUDED.tech_prefix, default_country_code = EXCLUDED.default_country_code, notes = EXCLUDED.notes, deleted_at = NULL
		RETURNING `+customerCols,
		c.Name, c.Status, c.RateGroupID, c.RouteGroupID, c.MaxConcurrentCalls, c.MaxCPS, c.AllowedCodecs, c.TechPrefix, c.DefaultCountryCode, c.Notes)
}
