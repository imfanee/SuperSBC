...
const customerCols = `id, name, status::text AS status, rate_group_id, route_group_id, max_concurrent_calls, max_cps,
	...
	media_mode::text AS media_mode, dtmf_mode::text AS dtmf_mode, srtp_mode::text AS srtp_mode, require_tls, stir_mode::text AS stir_mode, tls_subject, notes, created_at, updated_at`
...
func (s *Store) CustomerByIP(ctx context.Context, ip string, port int, transport string) (*IPMatch, error) {
	...
	rows, err := s.pool.Query(ctx, `
		SELECT c.id, c.name, c.status::text AS status, c.rate_group_id, c.route_group_id, c.max_concurrent_calls, c.max_cps,
		       ...
		       c.media_mode::text, c.dtmf_mode::text, c.srtp_mode::text, c.require_tls, c.stir_mode::text, c.tls_subject, c.notes, c.created_at, c.updated_at,
	...
	if err := rows.Scan(&c.ID, &c.Name, &c.Status, &c.RateGroupID, &c.RouteGroupID, &c.MaxConcurrentCalls, &c.MaxCPS,
		...
		&c.MediaMode, &c.DTMFMode, &c.SRTPMode, &c.RequireTLS, &c.STIRMode, &c.TLSSubject, &c.Notes, &c.CreatedAt, &c.UpdatedAt,
...
// Customers returns every live customer (used by the config renderer).
func (s *Store) Customers(ctx context.Context) ([]model.Customer, error) {
	return many[model.Customer](ctx, s.pool, `SELECT `+customerCols+` FROM customers WHERE deleted_at IS NULL ORDER BY name`)
}
...
func (s *Store) UpsertCustomer(ctx context.Context, c *model.Customer) (*model.Customer, error) {
	return one[model.Customer](ctx, s.pool, `
		...
		ON CONFLICT (name) DO UPDATE SET status = EXCLUDED.status, rate_group_id = EXCLUDED.rate_group_id, route_group_id = EXCLUDED.route_group_id,
		  ...
		  media_mode = EXCLUDED.media_mode, dtmf_mode = EXCLUDED.dtmf_mode, srtp_mode = EXCLUDED.srtp_mode, require_tls = EXCLUDED.require_tls, stir_mode = EXCLUDED.stir_mode, tls_subject = EXCLUDED.tls_subject, deleted_at = NULL
