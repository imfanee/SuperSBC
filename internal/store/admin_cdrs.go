...
func (f CDRFilter) conds() ([]string, []any) {
	...
	if f.SIPCallID != "" {
		add("d.sip_call_id = $%d", f.SIPCallID)
	}
	if f.NegativeOnly {
...
const cdrRowCols = `d.call_uuid, d.customer_id, d.carrier_id, d.src_ip::text AS src_ip, d.src_port, d.caller_number_raw, d.caller_number, d.called_number_raw, d.called_number,
	...
	d.attempts, d.failover_depth, d.codec_in, d.codec_out, d.media_mode, COALESCE(d.rtp_stats, 'null'::jsonb) AS rtp_stats, d.transport_in, d.transport_out, d.srtp_in, d.srtp_out, d.privacy, d.stir_status, d.stir_attest, d.sip_call_id, d.sbc_node, d.billed_at, d.billed_by, d.created_at, d.updated_at,
